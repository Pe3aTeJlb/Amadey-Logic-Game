...
public class Factorisator_V_2 {
    ...
    //массив термов
    private String[] initTerms = new String[0];
    ...
    public void PrepareData(String expression, int varcount) {
        ...
        initTerms = expression.split(splitSymbol);

        //Заполняем алфавит
        for(int i = 0; i < initTerms.length; i++){
            String[] buffer = initTerms[i].split(splitSymbol2);
            for(int j = 0; j < buffer.length; j++){
                if(!operands.contains(buffer[j])){
                    operands.add(buffer[j]);
                }
            }
        }
        ...
        System.out.println("alphabet " + operands.toString());
        ...
        for(int i = 0; i < initTerms.length; i++){
            ...
            int l = 0;
            while (!buffer[0].equals(operands.get(l))){
                binaryString += "0";
                l++;
            }
            ...
            for(int m = l; m < operands.size(); m++){
                if(operands.get(m).equals(buffer[z]) && z < buffer.length){
                    binaryString += "1";
                    if(z != buffer.length - 1){
                        z += 1;
                    }
                }else if(!operands.get(m).equals(buffer[z]) || m > buffer.length) {
                    binaryString += "0";
                }
            }
    ...
    private void Factorisation(){
        ...
        System.out.println("collisions " + collisionTerms.toString());
    ...
    private void ReconstructFunction(){
        ...
        ArrayList<Integer> globalOperands = new ArrayList<>();
        ...
        c = c.replace("(", "");
        c = c.replace(")", "");
        for(int i = 0; i < globalOperands.size(); i++){
        ...
        if(globalOperands.size() != 0){
            data.Delete(globalOperands);
            data.PrintCollisions();
        } else{
            //Если нет глобальных операндо, то чекним, есть ли операнды и термы, из которых нельзя ничего вынести
            unclaimedBuff = data.GetUnclaimedOperands(insideSeparator,interstitialSeparator, initTerms);
            if(unclaimedBuff.length() > 0)
                unclaimedBuff = removeByIndex(unclaimedBuff,unclaimedBuff.length()-1);

            System.out.println("unclaimed operands: " + unclaimedBuff);
        }
        ...
        if(data.positions.get(0).size() == 1){
            ...
            System.out.println("recurion start");
            //Уходим в рекурсию
    ...
    private String RecurcivePart(CustomData container, ArrayList<Integer> prevTerms){
        ...
        if(maxIndex == 1) {
            ...
            if(unclaimed.length() > 1){
                unclaimed = removeByIndex(unclaimed, unclaimed.length()-1);
            }
        ...
        if(unclaimed.length() > 0){
            tmp += interstitialSeparator + unclaimed;
        }
    ...
    static class CustomData{
        ...
        public void Delete(int index){
            System.out.println("delete index " + index +" "+ alphabet.get(index));
        ...
        public void PrintCollisions(){
            System.out.println("collisions: "+positions.toString());
        }

        //Удаляет операнды которые не входят в данный терм
        public void DeleteOutOfBounds(ArrayList<Integer> outOfBounds){
            System.out.println("delete out of bounds " + outOfBounds.toString());
            for(int i = positions.size()-1; i>=0; i--){
                if(Collections.disjoint(outOfBounds,positions.get(i))){
                    Delete(i);
                }
            }
        ...
        public void RemoveExclusive(ArrayList<ArrayList<Integer>> crossColision, ArrayList<Integer> container){

            System.out.println("remove exclusive: " + crossColision.toString() + " -|- " + container.toString());
            for (ArrayList<Integer> c: crossColision) {
                ArrayList<Integer> buff = new ArrayList<>();
                for(int i = 0; i < c.size(); i++){
                    if(container.contains(c.get(i))){
                    }else{
                        buff.add(i);
                    }
                }
